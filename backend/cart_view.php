...
<head>
  ...
  <style>
    ...
    @media (max-width: 719px) {
      ...
      table.cart tr { display:block; background:#fff; margin-bottom:14px; border-radius:10px; padding:10px; }
      table.cart td { display:block; width:100%; }
      table.cart td img.thumb { width:100%; height:auto; max-width:200px; }
      table.cart td + td { margin-top:8px; }
    ...
    @media (min-width: 1000px) {
      ...
      table.cart td img.thumb { width:80px; height:60px; }
...
<main class="cart-container">
  ...
  <?php } else { ?>
    ...
    <form action="/~cs6636089/GearZone/backend/cart.php" method="post">
      ...
      <div class="total-box">
        <div>
          <div>รวมทั้งสิ้น: <strong>฿<?php echo $grand; ?></strong></div>

          <?php if ($birth_ok) { ?>
            <div class="note" style="color:#d00;">
              ส่วนลดเดือนเกิด 12%: -฿<?php echo $discount; ?>
            </div>
            <div class="note" style="font-weight:bold; color:#000;">
              ยอดสุทธิ: ฿<?php echo $final; ?>
            </div>
          <?php } ?>

          <!-- <div class="note">*ราคายังไม่รวมค่าจัดส่ง</div> -->
        </div>
